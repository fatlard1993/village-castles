...
Villages with teeth. A Fabric mod that adds castle structures to village generation. Each biome gets its own architectural language: not palette swaps, but genuinely different shapes, proportions, and character. A desert citadel doesn't look like a taiga fortress with the blocks changed.
...
Villages generate completely normally, with vanilla center, streets, houses, villagers. After the jigsaw assembly finishes, a mixin intercepts the completed structure and attaches a castle to the village edge.
...
- **Terrain integration**: Height sampling fixed: the mixin queries `getFirstOccupiedHeight` at the castle's actual X/Z instead of using the village bounding-box minY. Terrain blending (smooth biome-appropriate foundations at edges) still needed in the generators.
...
- **No inhabitants**: castle NBTs ship beds and workstations (POI blocks) but zero baked entities, and no post-placement spawn code exists anywhere. Castles are empty buildings; quest dialogue references guards who aren't there. Undesigned feature, not a regression.
...
- **Pool injection into `town_centers`**: Castles replaced the village center. Bounding box too large; the jigsaw system couldn't attach streets. Villages had no houses or villagers.
...
- Must not break vanilla village generation, only augment it
